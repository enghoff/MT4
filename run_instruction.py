...
"""Type an instruction in English; the arm carries it out.
    ...
    python run_instruction.py                       # interactive prompt + window
...
Interactive is the default, and it is the shape ``ask_qwen.py`` uses: a prompt
line pinned to the bottom of the terminal, a background worker doing the slow
thing, and a window that keeps redrawing while it does. What differs is what
the worker does with the answer -- here it moves a real arm.
...
if __name__ == "__main__":
    ...
    # serial port, the window and the camera. This script now holds a
    # FrameStream for the whole session, which puts it in exactly the position
    # ask_qwen.py documents: OpenCV highgui can keep a Windows process alive
    # past the end of main, and a lingering process holds the capture device
    # open -- stranding the camera for every other vision script in the repo
